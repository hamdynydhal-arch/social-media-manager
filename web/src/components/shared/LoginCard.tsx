    ...
    >
      <div className="bg-navy-800/80 border border-gold-DEFAULT/15 rounded-2xl p-8 shadow-2xl">
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-3 bg-[#D4AF37] hover:bg-[#F4CE14] text-[#030C1B] font-bold py-3.5 px-6 rounded-xl transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed shadow-[0_0_30px_rgba(212,175,55,0.25)]"
        >
          {loading ? (
            <div className="w-5 h-5 border-2 border-[#030C1B]/30 border-t-[#030C1B] rounded-full animate-spin" />
          ) : (
          ...
          {loading ? "جارٍ المصادقة..." : "مصادقة وصول المصرّحين (Google)"}
        </button>
        ...
        {/* Security note */}
        <div className="flex items-start gap-3 mt-5 p-4 bg-white/[0.03] rounded-xl border border-gold-DEFAULT/10">
