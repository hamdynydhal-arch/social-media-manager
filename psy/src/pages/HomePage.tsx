...
export default function HomePage({ onSelectOcean, onSelectAttachment, onSelectSchema, onSelectSynthesis, onSelectSettings, onSelectIntake }: HomePageProps) {
  return (
    <div className="min-h-screen bg-nafees-cream flex flex-col items-center">
      ...
      {/* Dark navy header */}
      <div className="w-full bg-nafees-navy text-center px-4 pt-10 pb-10 rounded-b-[2.5rem]" dir="rtl">
        {/* Top action row */}
        <div className="flex items-center justify-between mb-6 max-w-md mx-auto">
          <button
            onClick={onSelectSettings}
            className="flex items-center gap-1.5 bg-white/15 hover:bg-white/25 border border-white/20 px-3 py-1.5 rounded-full text-nafees-cream text-xs font-semibold transition-all duration-200 active:scale-95"
