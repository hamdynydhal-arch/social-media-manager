export type SchemaTier = 'core' | 'deep';

interface SchemaTierModalProps {
  onSelect: (tier: SchemaTier) => void;
  onCancel: () => void;
}

export default function SchemaTierModal({ onSelect, onCancel }: SchemaTierModalProps) {
  return (
    <div
      ...
      >
        ...
        <div className="px-5">
          ...
          <p className="text-xs text-nafees-warm text-center mb-5 leading-relaxed">
            كلا المستويين يقيسان المخططات السبعة المستندة إلى نظرية يونج المعرفية
          </p>

          <button
            onClick={() => onSelect('core')}
            className="w-full bg-white border-2 border-nafees-sage/40 hover:border-nafees-sage rounded-2xl p-5 mb-3 text-right active:scale-[0.98] transition-all duration-200 shadow-sm"
          >
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-2xl bg-nafees-sage/15 flex items-center justify-center flex-shrink-0 text-2xl">
                ⚡
              </div>
