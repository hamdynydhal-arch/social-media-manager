export type SocialPatternsTier = 'core' | 'deep';

interface SocialPatternsTierModalProps {
  onSelect: (tier: SocialPatternsTier) => void;
  onCancel: () => void;
}

export default function SocialPatternsTierModal({ onSelect, onCancel }: SocialPatternsTierModalProps) {
  return (
    <div
      ...
      >
        ...
        <div className="px-5">
          ...
          <p className="text-xs text-nafees-warm text-center mb-5 leading-relaxed">
            كلا المستويين يقيسان أبعادك الاجتماعية الأربعة عبر مؤشرات دقيقة
          </p>

          <button
            onClick={() => onSelect('core')}
            className="w-full bg-white border-2 border-nafees-blue/30 hover:border-nafees-blue rounded-2xl p-5 mb-3 text-right active:scale-[0.98] transition-all duration-200 shadow-sm"
          >
